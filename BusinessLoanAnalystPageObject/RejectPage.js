class BusinessReject {
  constructor(page) {
    ...
    this.Rejectedreason = page.locator(
      '[data-testid="Reason for Rejection:text-area:control"]'
    );

    this.ConformButton = page.locator(
      "xpath=/html/body/app-root/div/div[1]/div/main/div/div/div[2]/div[1]/div[2]/div/article/div/li/article/div/form/div[1]/div/div/fieldset/div/div/div/div[8]/fieldset/div/div/div/div[1]/div[6]/fieldset/div[1]/div/label/div"
    );
  ...
  async Approve(feedback, Decision, Reason) {
    await this.AnalystFeedback.fill(feedback);
    await this.Approval.selectOption(Decision);
    await this.Rejectedreason.fill(Reason);
    await this.ConformButton.click();
    await this.page.screenshot({ path: "screenshot4.png" });
    await this.SubmitButton.click();
  }
}

module.exports = { BusinessReject };
